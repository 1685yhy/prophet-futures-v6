...
V33 = 紧止损宽止盈 + v31模型 + 模型动态决策
独立状态文件 paper_state_v33.json
"""
...
SYMBOLS = {
    'lh2609': {
        ...
        'atr_stop': 0.5,  # V33 紧止损
        'rr': 6.0,         # V33 宽止盈
        'add_conf': 0.70,  # V33 更难加仓
        'add_atr': 3.0,    # 加仓: 浮盈>3ATR
        'reduce_conf': 0.45,  # V33 更早减仓
        'reverse_conf': 0.0,  # V33 反手关闭(0717网格结论:+71pp)
        'trail_atr': 1.5,     # V33 更早追踪
        'be_atr': 0.8,        # V33 更早保本
        'min_hold': 5,        # 最小持仓bar
    },
}
...
STATE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'paper_state_v33.json')
...
def main():
    ...
    # PID锁: 防止重复启动
    import fcntl
    lock_fd = open('/tmp/paper_v33.lock', 'w')
    try:
    ...
    print('=' * 60)
    print('  Prophet V33 — 动态加仓/减仓/反手引擎')
    ...
    # Load models
    models = {}
# V33 uses newly trained model from v5 backtest
    ...
    print('\n' + '='*60)
    print('  V33 引擎就绪 (独立状态: paper_state_v33.json)')
    print('='*60 + '\n')
    ...
    while running:
        ...
        print('\n[%s] V33 Scanning...' % now.strftime('%H:%M:%S'))
            ...
            for pos in pos_list:
                ...
                if d == 'LONG':
                    ...
                    if low <= eff_stop:
                        ...
                        emoji = '🟢' if pnl>0 else '🔴'
                        print('  %s [V33-STOP] %s %s %d手 @%.0f→%.0f PnL=%+.0f 余额=¥%s' % (
                            emoji, sym_key, d, vol, entry, ep, pnl, format(int(state['cash']), ',')))
                        log_event('V33 STOP %s %s %d手 @%s→%s PnL=%+.0f' % (sym_key, d, vol, entry, ep, pnl))
                        ...
                        try: send_alert('%s [V33] 止损 | %s' % (emoji, sym_key),
                        ...
                        state.setdefault('trades', []).append(trade)
                        print('  🔴 [V33-REV] %s %s→反手平仓 @%.0f PnL=%+.0f' % (sym_key, d, price, pnl))
                        log_event('V33 REV %s %s %d手 @%s→%s PnL=%+.0f' % (sym_key, d, vol, entry, price, pnl))
                        log_trade('V32', sym_key, 'CLOSE', d, entry, price, vol, pnl,
                                  '反手|REVERSE', '概率=%.3f<%.2f_反手阈值' % (prob, cfg['reverse_conf']),
                                  state['cash'], state['cash'])
                        try: send_alert('🔴 [V33] 反手 | %s' % sym_key,
                        ...
                        state.setdefault('trades', []).append(trade)
                        print('  🟡 [V33-RED] %s 减仓 %d→%d手 @%.0f PnL=%+.0f' % (sym_key, vol, vol-cut, price, pnl))
                        log_event('V33 RED %s %s %d→%d手 @%.0f PnL=%+.0f' % (sym_key, d, vol, vol-cut, price, pnl))
                    ...
                    if high >= eff_stop:
                        ...
                        state.setdefault('trades', []).append(trade)
                        emoji = '🟢' if pnl>0 else '🔴'
                        print('  %s [V33-STOP] %s %s %d手 @%.0f→%.0f PnL=%+.0f' % (emoji, sym_key, d, vol, entry, ep, pnl))
                        log_event('V33 STOP %s %s %d手 @%s→%s PnL=%+.0f' % (sym_key, d, vol, entry, ep, pnl))
                        log_trade('V32', sym_key, 'CLOSE', d, entry, ep, vol, pnl,
                                  '止损|STOP', '有效止损=%.1f_硬止损=%.1f_追踪=%.1f_概率=%.3f' % (eff_stop, hard_stop, trail, prob),
                                  state['cash'], state['cash'])
        ...
        for sym_key in SYMBOLS:
            ...
            conf = prob if prob > 0.5 else 1-prob
            if conf < 0.55: continue  # V33 入场置信度门槛
            conf = prob if prob > 0.5 else 1-prob
            ...
            if not cur_positions:
                ...
                if sd == 'LONG':
                    ...
                    print('  🟢 [V33] 开多 %s %d手 @%.0f 止损%.0f 保证金¥%.1f万' % (sym_key, ps, price, entry_stop, marg/10000))
                    log_event('V33 OPEN %s LONG %d手 @%s STOP=%s' % (sym_key, ps, price, entry_stop))
                    log_trade('V32', sym_key, 'OPEN', 'LONG', price, 0, ps, 0,
                              '模型信号|SIGNAL', '概率=%.3f_置信=%.2f_ATR=%.1f_stop=%.0f' % (prob, conf, atr, entry_stop),
                              state['cash'], state['cash'])
                    try: send_alert('🟢 [V33] 开多 | %s' % sym_key,
                    ...
                    traded_today.add(sym_key)
                    marg = ps * price * cfg['multiplier'] * 0.15
                    print('  🔴 [V33] 开空 %s %d手 @%.0f 止损%.0f 保证金¥%.1f万' % (sym_key, ps, price, entry_stop, marg/10000))
                    log_event('V33 OPEN %s SHORT %d手 @%s STOP=%s' % (sym_key, ps, price, entry_stop))
                    log_trade('V32', sym_key, 'OPEN', 'SHORT', price, 0, ps, 0,
                              '模型信号|SIGNAL', '概率=%.3f_置信=%.2f_ATR=%.1f_stop=%.0f' % (prob, conf, atr, entry_stop),
                              state['cash'], state['cash'])
                    try: send_alert('🔴 [V33] 开空 | %s' % sym_key,
                        '%d手 @%.0f\n止损%.0f\n保证金¥%.1f万' % (ps, price, entry_stop, marg/10000),
                        color='blue', pin=True)
                    except: pass
            else:
                ...
                if conf > cfg['add_conf'] and pnl_atr > cfg['add_atr']:
                    ...
                    print('  ➕ [V33] 加仓 %s +%d手 @%.0f 共%d手 浮盈%.1fATR' % (sym_key, ps, price, total_now, pnl_atr))
                    log_event('V33 ADD %s +%d手 @%s 共%d手' % (sym_key, ps, price, total_now))
                    log_trade('V32', sym_key, 'ADD', existing_dir, price, 0, ps, 0,
                              '加仓|ADD', '置信=%.2f>%.2f_浮盈ATR=%.2f>%.1f' % (conf, cfg['add_conf'], pnl_atr, cfg['add_atr']),
                              state['cash'], state['cash'])
                    try: send_alert('➕ [V33] 加仓 | %s' % sym_key,
