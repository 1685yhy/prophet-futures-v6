#!/usr/bin/env python3
"""LH fine grid: Struct stop + RR + vol sizing, 1000WF"""
import numpy as np, pandas as pd, json, time
import akshare as ak, xgboost as xgb

df = ak.futures_main_sina(symbol='LH0', start_date='20210101', end_date='20260620')
...
CAP = 300000; N_WF = 1000; SN = [18, 20, 22, 25]; RRs = [2, 3, 4]
n = len(df); ts = int(n * 0.7)

# Precompute
F = np.zeros((n, 20), dtype=np.float32); A = np.zeros(n); V = np.zeros(n, dtype=bool)
...
for sn in SN:
    for rr_val in RRs:
        ...
        print(f'N={sn} RR={rr_val} {total}t WR={wr:.1f}% PF={pf:.2f} PnL={tp/10000:+.1f}万 AR={ar*100:.1f}% DD={max_dd:.0f}% CM={cm:.2f} ns={npct:.1f}% {elapsed:.0f}s')
        results.append({'sn': sn, 'rr': rr_val, 't': total, 'wr': round(wr,1), 'pf': round(pf,2),
                       'pnl': round(tp,0), 'ar': round(ar*100,1), 'dd': round(max_dd,1),
                       'cm': round(cm,2), 'ns': round(npct,1)})

results.sort(key=lambda x: -x['cm'])
with open('/home/a/prophet_futures/prophet_futures/lh_fine.json', 'w') as f:
    json.dump(results, f, indent=2, default=str)
print('\nBEST:')
for r in results[:3]:
    print(f'  N={r["sn"]} RR={r["rr"]} PnL={r["pnl"]/10000:+.1f}万 AR={r["ar"]}% DD={r["dd"]}% CM={r["cm"]}')
