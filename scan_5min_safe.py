#!/usr/bin/env python3
"""5分钟扫描 v4 safe — 使用子进程隔离模型加载以避免 glibc 2.39 ld.so bug"""
...
def predict_in_subprocess(sym_key, price, atr, cfg):
    """Run model prediction in a subprocess to isolate glibc crash"""
    code = f'''
import sys, os, json, pickle, numpy as np
sys.path.insert(0, "{os.path.dirname(os.path.abspath(__file__))}")
sys.path.insert(0, "/tmp/test_venv/lib/python3.11/site-packages")
from realtime_data import get_daily_history, build_features, SYMBOL_MAP

info = SYMBOL_MAP.get("{sym_key}")
MODEL_DIR = "{MODEL_DIR}"
PRED_HISTORY_FILE = "{PRED_HISTORY_FILE}"
sym_key = "{sym_key}"
price = {price}
atr = {atr}
cfg = {json.dumps(cfg)}
...
'''
    try:
        result = subprocess.run(
            ['/tmp/test_venv/bin/python3.11', '-c', code],
            capture_output=True, text=True, timeout=60
        )
        if result.returncode == 0 and result.stdout.startswith('SIGNAL:'):
            return result.stdout[7:].strip()
    except:
        pass
    return None
...
def main():
    ...
    for sym_key, cfg in SYMBOLS.items():
        ...
        # Model prediction via subprocess
        signal_text = predict_in_subprocess(sym_key, price, atr, cfg)
        if signal_text:
