#!/usr/bin/env python3
"""Prophet Futures — Paper Trading Engine v26
...
SYMBOLS = {
    ...
    'jm2609': {
        ...
        'max_pos': 4,
        # V26 dynamic exit params (JM: wider, slower — higher volatility)
        'hard_atr': 1.8,       # 硬止损 ATR倍数 (JM波动大)
        'trail_atr': 3.0,      # 移动止损启动: 盈利>3ATR
        'be_atr': 2.0,         # 保本启动: 盈利>2ATR
        'trail_dist': 2.5,     # 移动止损距离
        'rr': 3.5,             # 止盈风险比 (V26修复: 不应用trail_atr+trail_dist当RR)
...
# ===== V26 DYNAMIC EXIT =====
def check_v26_exit(sym_key, pos, models, df, now):
    """
    V26 dynamic exit logic:
...
def main():
    ...
    if state['positions']:
        for k, v in state['positions'].items():
            cfg = SYMBOLS.get(k, {})
            # Initialize v26 tracking fields
    ...
    while running:
        ...
        # ===== CHECK EXISTING POSITIONS (V26 DYNAMIC EXIT) =====
        for sym_key in list(state['positions'].keys()):
            pos = state['positions'][sym_key]
            cfg = SYMBOLS.get(sym_key)
            if not cfg: continue
            ...
            should_exit, exit_price, reason = check_v26_exit(sym_key, pos, models, df, now)
        ...
        for sym_key, cfg in SYMBOLS.items():
            ...
            # Correct TP using actual RR config (V26 fix: was using trail_atr+trail_dist)
