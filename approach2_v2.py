#!/usr/bin/env python3
"""Approach 2 v2: 30-min WF + full 6-month data, proper WF validation"""
...
ne = 200; d = 5; lr = 0.05; n_wf = 50  # Walk-forward windows
...
print(f'Approach 2 v2: 30-min WF, {len(m30)} bars, {n_wf} windows, {total_combos} combos')
print(f'Train: ~{int(len(m30)*0.7)} bars  Test: ~{int(len(m30)*0.3/n_wf)} bars/window')
...
for sm in SM:
    for rr_val in RR:
        for conf in CF:
            ...
            train_size = int(len(m30) * 0.7)
            step = max(1, (len(m30) - train_size) // n_wf)
            
            for wf in range(min(n_wf, (len(m30) - train_size) // step)):
                sp = train_size + wf * step
                if sp + step > len(m30): break
                
                train_df = m30.iloc[:sp]
                test_df = m30.iloc[sp:sp+step]
