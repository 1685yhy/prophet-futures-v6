#!/usr/bin/env python3
"""JM/RM: trend-following + struct stop optimization, 2000WF"""
import numpy as np, pandas as pd, json, time
import akshare as ak, xgboost as xgb

# Trend-following config (no reversal, shorter windows)
SYM = {
    'JM': ('JM0', 0.0011, False, 100, 4, 0.03, 60, 60, 5),   # same model, just changed label
    'RM': ('RM0', 0.0011, False, 100, 5, 0.03, 60, 10, 20),   # trend, shorter window
}
...
def precompute(df, win=60):
    ...
    for i in range(n-1):
        if V[i]: L[i] = 1 if df.iloc[i+1]['close'] > df.iloc[i]['close'] else 0  # TREND label
    return F, A, V, L
