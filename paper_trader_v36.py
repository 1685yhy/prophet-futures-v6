...
V36 = V25 ATR基底 + 模型动态决策
独立状态文件 paper_state_v36.json
"""
...
SYMBOLS = {
    'lh2609': {
        ...
        'atr_stop': 2.0,  # V36 宽止损(0718网格最优)
        'rr': 4.0,         # 止盈风险比
        'add_conf': 0.65,  # 加仓: 置信度>65%
        'add_atr': 2.0,    # 加仓: 浮盈>2ATR
        'reduce_conf': 0.55,  # 减仓: 置信度<55%
        'reverse_conf': 0.35, # 反手: prob<0.35做多转空
        'trail_atr': 2.0,     # 移动止损启动ATR
        'be_atr': 1.0,        # 保本ATR
        'min_hold': 3,        # 最小持仓bar
    },
}
...
STATE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'paper_state_v36.json')
...
def main():
    ...
    # PID锁: 防止重复启动
    import fcntl
    lock_fd = open('/tmp/paper_v36.lock', 'w')
    try:
    ...
    print('=' * 60)
    print('  Prophet V36 — 动态加仓/减仓/反手引擎')
    ...
    for sym_key in SYMBOLS:
        mp = os.path.join(MODEL_DIR, 'v36_bal_xgb.pkl' if sym_key == 'lh2609' else sym_key+'_xgb.pkl')
    ...
    print('\n' + '='*60)
    print('  V36 引擎就绪 (独立状态: paper_state_v36.json)')
    print('='*60 + '\n')

    # 恢复检查：启动时补执行已穿止损
    try:
        from trader_recovery import run_recovery
        run_recovery(STATE_FILE, SYMBOLS, 'V28')
    except Exception as e:
        print(f"  [V36] 恢复检查跳过: {e}")
    ...
    while running:
        ...
        print('\n[%s] V36 Scanning...' % now.strftime('%H:%M:%S'))
            ...
            for pos in pos_list:
                ...
                if d == 'LONG':
                    ...
                    if low <= eff_stop:
                        ...
                        emoji = '🟢' if pnl>0 else '🔴'
                        print('  %s [V36-STOP] %s %s %d手 @%.0f→%.0f PnL=%+.0f 余额=¥%s' % (
                            emoji, sym_key, d, vol, entry, ep, pnl, format(int(state['cash']), ',')))
                        log_event('V36 STOP %s %s %d手 @%s→%s PnL=%+.0f' % (sym_key, d, vol, entry, ep, pnl))
                        log_trade('V28', sym_key, 'CLOSE', d, entry, ep, vol, pnl,
                                  '止损|STOP', '有效止损=%.1f_硬止损=%.1f_追踪=%.1f_概率=%.3f' % (eff_stop, hard_stop, trail, prob),
                                  state['cash'], state['cash'])
                        try: send_alert('%s [V36] 止损 | %s' % (emoji, sym_key),
                        ...
                        state.setdefault('trades', []).append(trade)
                        print('  🔴 [V36-REV] %s %s→反手平仓 @%.0f PnL=%+.0f' % (sym_key, d, price, pnl))
                        log_event('V36 REV %s %s %d手 @%s→%s PnL=%+.0f' % (sym_key, d, vol, entry, price, pnl))
                        log_trade('V28', sym_key, 'CLOSE', d, entry, price, vol, pnl,
                                  '反手|REVERSE', '概率=%.3f<%.2f_反手阈值' % (prob, cfg['reverse_conf']),
                                  state['cash'], state['cash'])
                        try: send_alert('🔴 [V36] 反手 | %s' % sym_key,
                        ...
                        state.setdefault('trades', []).append(trade)
                        print('  🟡 [V36-RED] %s 减仓 %d→%d手 @%.0f PnL=%+.0f' % (sym_key, vol, vol-cut, price, pnl))
                        log_event('V36 RED %s %s %d→%d手 @%.0f PnL=%+.0f' % (sym_key, d, vol, vol-cut, price, pnl))
                        log_trade('V28', sym_key, 'REDUCE', d, entry, price, cut, pnl,
                    ...
                    if high >= eff_stop:
                        ...
                        state.setdefault('trades', []).append(trade)
                        emoji = '🟢' if pnl>0 else '🔴'
                        print('  %s [V36-STOP] %s %s %d手 @%.0f→%.0f PnL=%+.0f' % (emoji, sym_key, d, vol, entry, ep, pnl))
                        log_event('V36 STOP %s %s %d手 @%s→%s PnL=%+.0f' % (sym_key, d, vol, entry, ep, pnl))
                        log_trade('V28', sym_key, 'CLOSE', d, entry, ep, vol, pnl,
                                  '止损|STOP', '有效止损=%.1f_硬止损=%.1f_追踪=%.1f_概率=%.3f' % (eff_stop, hard_stop, trail, prob),
                                  state['cash'], state['cash'])
        ...
        for sym_key in SYMBOLS:
            ...
            # V36: MA90趋势过滤器(0718网格: 均线上禁空/均线下禁多)
            if len(df) >= 90:
                ma90 = float(df['close'].iloc[-90:].mean())
                if price > ma90 and sd == 'SHORT': continue
                if price < ma90 and sd == 'LONG': continue
            ...
            if not cur_positions:
                ...
                if sd == 'LONG':
                    ...
                    print('  🟢 [V36] 开多 %s %d手 @%.0f 止损%.0f 保证金¥%.1f万' % (sym_key, ps, price, entry_stop, marg/10000))
                    log_event('V36 OPEN %s LONG %d手 @%s STOP=%s' % (sym_key, ps, price, entry_stop))
                    log_trade('V28', sym_key, 'OPEN', 'LONG', price, 0, ps, 0,
                              '模型信号|SIGNAL', '概率=%.3f_置信=%.2f_ATR=%.1f_stop=%.0f' % (prob, conf, atr, entry_stop),
                              state['cash'], state['cash'])
                    try: send_alert('🟢 [V36] 开多 | %s' % sym_key,
                    ...
                    traded_today.add(sym_key)
                    marg = ps * price * cfg['multiplier'] * 0.15
                    print('  🔴 [V36] 开空 %s %d手 @%.0f 止损%.0f 保证金¥%.1f万' % (sym_key, ps, price, entry_stop, marg/10000))
                    log_event('V36 OPEN %s SHORT %d手 @%s STOP=%s' % (sym_key, ps, price, entry_stop))
                    log_trade('V28', sym_key, 'OPEN', 'SHORT', price, 0, ps, 0,
                              '模型信号|SIGNAL', '概率=%.3f_置信=%.2f_ATR=%.1f_stop=%.0f' % (prob, conf, atr, entry_stop),
                              state['cash'], state['cash'])
                    try: send_alert('🔴 [V36] 开空 | %s' % sym_key,
                        '%d手 @%.0f\n止损%.0f\n保证金¥%.1f万' % (ps, price, entry_stop, marg/10000),
                        color='blue', pin=True)
                    except: pass
            else:
                ...
                if conf > cfg['add_conf'] and pnl_atr > cfg['add_atr']:
                    ...
                    print('  ➕ [V36] 加仓 %s +%d手 @%.0f 共%d手 浮盈%.1fATR' % (sym_key, ps, price, total_now, pnl_atr))
                    log_event('V36 ADD %s +%d手 @%s 共%d手' % (sym_key, ps, price, total_now))
                    log_trade('V28', sym_key, 'ADD', existing_dir, price, 0, ps, 0,
                              '加仓|ADD', '置信=%.2f>%.2f_浮盈ATR=%.2f>%.1f' % (conf, cfg['add_conf'], pnl_atr, cfg['add_atr']),
                              state['cash'], state['cash'])
                    try: send_alert('➕ [V36] 加仓 | %s' % sym_key,
