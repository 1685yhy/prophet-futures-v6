...
V35 = 紧止损宽止盈 + v31模型 + 模型动态决策
独立状态文件 paper_state_v35.json
"""
...
SYMBOLS = {
    'lh2609': {
        ...
        'atr_stop': 1.5,  # V35 与回测引擎一致
        'rr': 4.0,         # V35 与回测引擎一致
        'add_conf': 0.65,  # V35 与回测引擎一致
        'add_atr': 2.0,    # 加仓: 浮盈>2ATR
        'reduce_conf': 0.55,  # V35 与回测引擎一致
        'reverse_conf': 0.0,  # V35 反手关闭
        'trail_atr': 1.5,     # 追踪启动
        'be_atr': 0.8,        # 保本启动
        'min_hold': 3,        # 最小持仓bar
    },
...
STATE_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'paper_state_v35.json')
...
def build_features(df, idx, window=60):
    """V35: 60日×6维序列展平(360维) — 与v35_backtest._seq_matrix一致"""
    SEQ, NF = 60, 6
    if idx < SEQ + 10: return None
    c = df['close'].values.astype(float); o = df['open'].values.astype(float)
    h = df['high'].values.astype(float); l = df['low'].values.astype(float)
    v = df['volume'].values.astype(float); oi = df['oi'].values.astype(float)
    m = np.zeros((idx + 1, NF), dtype=np.float32)
    for i in range(max(1, idx - SEQ), idx + 1):
    ...
    return m[idx-SEQ+1:idx+1].flatten()
...
def main():
    ...
    # PID锁: 防止重复启动
    import fcntl
    lock_fd = open('/tmp/paper_v35.lock', 'w')
    try:
    ...
    print('=' * 60)
    print('  Prophet V35 — 动态加仓/减仓/反手引擎')
    print('  Capital: ¥%s  |  %s' % (format(CAPITAL, ','), datetime.now().strftime('%Y-%m-%d %H:%M')))
    print('  LH: LSTM序列模型 ATR×1.5/RR4 反手OFF (回测+999%)')
    print('=' * 60)

    # Load models — V35: LSTM(torch)
    models = {}
    from v35_torch_model import TorchSeqClassifier, _LSTM
    import torch as _torch
    mp = os.path.join(MODEL_DIR, 'v35_lstm.pt')
    if os.path.exists(mp):
        _clf = TorchSeqClassifier()
        _clf.model = _LSTM()
        _clf.model.load_state_dict(_torch.load(mp, map_location='cpu'))
        _clf.model.eval()
        models['lh2609'] = _clf
        print('  lh2609: LSTM loaded')
    ...
    print('\n' + '='*60)
    print('  V35 引擎就绪 (独立状态: paper_state_v35.json)')
    print('='*60 + '\n')
    ...
    while running:
        ...
        print('\n[%s] V35 Scanning...' % now.strftime('%H:%M:%S'))
            ...
            for pos in pos_list:
                ...
                if d == 'LONG':
                    ...
                    if low <= eff_stop:
                        ...
                        emoji = '🟢' if pnl>0 else '🔴'
                        print('  %s [V35-STOP] %s %s %d手 @%.0f→%.0f PnL=%+.0f 余额=¥%s' % (
                            emoji, sym_key, d, vol, entry, ep, pnl, format(int(state['cash']), ',')))
                        log_event('V35 STOP %s %s %d手 @%s→%s PnL=%+.0f' % (sym_key, d, vol, entry, ep, pnl))
                        ...
                        try: send_alert('%s [V35] 止损 | %s' % (emoji, sym_key),
                        ...
                        state.setdefault('trades', []).append(trade)
                        print('  🔴 [V35-REV] %s %s→反手平仓 @%.0f PnL=%+.0f' % (sym_key, d, price, pnl))
                        log_event('V35 REV %s %s %d手 @%s→%s PnL=%+.0f' % (sym_key, d, vol, entry, price, pnl))
                        log_trade('V32', sym_key, 'CLOSE', d, entry, price, vol, pnl,
                                  '反手|REVERSE', '概率=%.3f<%.2f_反手阈值' % (prob, cfg['reverse_conf']),
                                  state['cash'], state['cash'])
                        try: send_alert('🔴 [V35] 反手 | %s' % sym_key,
                        ...
                        state.setdefault('trades', []).append(trade)
                        print('  🟡 [V35-RED] %s 减仓 %d→%d手 @%.0f PnL=%+.0f' % (sym_key, vol, vol-cut, price, pnl))
                        log_event('V35 RED %s %s %d→%d手 @%.0f PnL=%+.0f' % (sym_key, d, vol, vol-cut, price, pnl))
                    ...
                    if high >= eff_stop:
                        ...
                        state.setdefault('trades', []).append(trade)
                        emoji = '🟢' if pnl>0 else '🔴'
                        print('  %s [V35-STOP] %s %s %d手 @%.0f→%.0f PnL=%+.0f' % (emoji, sym_key, d, vol, entry, ep, pnl))
                        log_event('V35 STOP %s %s %d手 @%s→%s PnL=%+.0f' % (sym_key, d, vol, entry, ep, pnl))
                        log_trade('V32', sym_key, 'CLOSE', d, entry, ep, vol, pnl,
                                  '止损|STOP', '有效止损=%.1f_硬止损=%.1f_追踪=%.1f_概率=%.3f' % (eff_stop, hard_stop, trail, prob),
                                  state['cash'], state['cash'])
        ...
        for sym_key in SYMBOLS:
            ...
            conf = prob if prob > 0.5 else 1-prob
            if conf < 0.55: continue  # V35 入场置信度门槛
            conf = prob if prob > 0.5 else 1-prob
            ...
            if not cur_positions:
                ...
                if sd == 'LONG':
                    ...
                    print('  🟢 [V35] 开多 %s %d手 @%.0f 止损%.0f 保证金¥%.1f万' % (sym_key, ps, price, entry_stop, marg/10000))
                    log_event('V35 OPEN %s LONG %d手 @%s STOP=%s' % (sym_key, ps, price, entry_stop))
                    log_trade('V32', sym_key, 'OPEN', 'LONG', price, 0, ps, 0,
                              '模型信号|SIGNAL', '概率=%.3f_置信=%.2f_ATR=%.1f_stop=%.0f' % (prob, conf, atr, entry_stop),
                              state['cash'], state['cash'])
                    try: send_alert('🟢 [V35] 开多 | %s' % sym_key,
                    ...
                    traded_today.add(sym_key)
                    marg = ps * price * cfg['multiplier'] * 0.15
                    print('  🔴 [V35] 开空 %s %d手 @%.0f 止损%.0f 保证金¥%.1f万' % (sym_key, ps, price, entry_stop, marg/10000))
                    log_event('V35 OPEN %s SHORT %d手 @%s STOP=%s' % (sym_key, ps, price, entry_stop))
                    log_trade('V32', sym_key, 'OPEN', 'SHORT', price, 0, ps, 0,
                              '模型信号|SIGNAL', '概率=%.3f_置信=%.2f_ATR=%.1f_stop=%.0f' % (prob, conf, atr, entry_stop),
                              state['cash'], state['cash'])
                    try: send_alert('🔴 [V35] 开空 | %s' % sym_key,
                        '%d手 @%.0f\n止损%.0f\n保证金¥%.1f万' % (ps, price, entry_stop, marg/10000),
                        color='blue', pin=True)
                    except: pass
            else:
                ...
                if conf > cfg['add_conf'] and pnl_atr > cfg['add_atr']:
                    ...
                    print('  ➕ [V35] 加仓 %s +%d手 @%.0f 共%d手 浮盈%.1fATR' % (sym_key, ps, price, total_now, pnl_atr))
                    log_event('V35 ADD %s +%d手 @%s 共%d手' % (sym_key, ps, price, total_now))
                    log_trade('V32', sym_key, 'ADD', existing_dir, price, 0, ps, 0,
                              '加仓|ADD', '置信=%.2f>%.2f_浮盈ATR=%.2f>%.1f' % (conf, cfg['add_conf'], pnl_atr, cfg['add_atr']),
                              state['cash'], state['cash'])
                    try: send_alert('➕ [V35] 加仓 | %s' % sym_key,
